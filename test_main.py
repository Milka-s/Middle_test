...
def analyzer(tmp_path):
    ...
    test_file.write_text("Ukraine, 603628, 41000000\nFrance, 551695, 67000000\nPoland, 312685, 38000000",
                         encoding="utf-8")
...
@pytest.mark.parametrize("sort_type, reverse, expected_first_country", [
    ("area", False, "Poland"),
    ("area", True, "Ukraine"),
    ("population", False, "Poland"),
    ("population", True, "France")
])
def test_sorting(analyzer, sort_type, reverse, expected_first_country):
    analyzer.read_file()
    ...
    assert sorted_data[0]['name'] == expected_first_country